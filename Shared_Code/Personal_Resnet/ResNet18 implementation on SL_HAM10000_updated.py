...
net_glob_client = ResNetClient(block, [2, 2, 2, 2, 2, 2], image_channels=3)
...
class ResNetServer(nn.Module):
    def __init__(self, block, layers, in_channels, num_classes):
        ...
        self.layer5 = self._make_layer(block, layers[4], out_channels = 1024, stride = 2)
        self.layer6 = self._make_layer(block, layers[5], out_channels = 2048, stride =2)
...
net_glob_server = ResNetServer(block, [2,2,2,2,2,2], client_in_channels, 7) #7 is my numbr of classes
if torch.cuda.device_count() > 1:
    print("We use",torch.cuda.device_count(), "GPUs")
...
"""class SplitResNet18(nn.Module):
    def __init__(self,image_channels,num_classes):
        ...
        self.client = ResNetClient(block, [2, 2, 2, 2], image_channels)
        in_channels = self.client.in_channels
        self.server = ResNetServer(block, [2, 2, 2, 2], in_channels, num_classes)
        
    def forward(self, x):
        client_output = self.client(x)
        server_output = self.server(client_output)
